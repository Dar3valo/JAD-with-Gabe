	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (userWithToken != null) {
				...
				SimpleDateFormat formatter = new SimpleDateFormat("MMMM dd, yyyy 'at' hh:mm a z"); // Using 12-hour
																									// format with AM/PM
				String formattedTime = formatter.format(tokenExpiryTime);
				String resetLink = request.getRequestURL().toString().replace("HandleForgetPasswordServlet",
						"public/HTML/resetPassword.jsp") + "?token=" + resetToken;
